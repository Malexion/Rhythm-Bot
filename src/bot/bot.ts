        ...
        map.on(
            ...
            (cmd: SuccessfulParsedMessage<Message>, msg: Message) => {
                ...
                if (msg.guild)
                    phrases = phrases.concat(
                        msg.guild.emojis.cache.array().map((x) => x.name)
                    );
                ...
                (cmd: SuccessfulParsedMessage<Message>, msg: Message) => {
                    joinUserChannel(msg)
                        .then((connection) => {
                            ...
                            msg.channel.send(
                                createInfoEmbed(
                                    `Joined Channel: ${connection.channel.name}`
                                )
                            );
            ...
            .on(
                ...
                (cmd: SuccessfulParsedMessage<Message>, msg: Message) => {
                    ...
                    this.client.voice.connections.forEach((conn) => {
                        ...
                        msg.channel.send(
                            createInfoEmbed(
                                `Disconnecting from channel: ${conn.channel.name}`
                            )
                        );
                    });
                }
                    ...
                    new Promise<void>((done) => {
                        if (!this.player.connection) {
                            joinUserChannel(msg).then((conn) => {
                                ...
                                msg.channel.send(
                                    createInfoEmbed(
                                        `Joined Channel: ${conn.channel.name}`
                                    )
                                );
                                done();
                            });
                ...
                (cmd: SuccessfulParsedMessage<Message>, msg: Message) => {
                    ...
                    if (this.player.playing && this.player.dispatcher) {
                        let elapsed = secondsToTimestamp(
                            this.player.dispatcher.totalStreamTime / 1000
                        );
                        msg.channel.send(
                            createInfoEmbed(
                                'Time Elapsed',
                        ...
                        (err, result) => {
                            result.videos.slice(0, 3).forEach((v, idx) => {
                                ...
                                msg.channel
                                    .send(embed)
                                    .then((m) =>
                                        m.react(this.config.emojis.addSong)
                                    );
                            });
                        }
                    );
            ...
            .on(
                ...
                (cmd: SuccessfulParsedMessage<Message>, msg: Message) => {
                    ...
                    if (items.length > 0)
                        ...
                        msg.channel.send(
                            createInfoEmbed(`There are no songs in the queue.`)
                        );
                }
            )
            .on(
            ...
            .on(
                ...
                (cmd: SuccessfulParsedMessage<Message>, msg: Message) => {
                    ...
                    msg.channel.send(
                        createInfoEmbed(
                            `Volume is at ${this.player.getVolume()}`
                        )
                    );
                }
            )
            .on(
                ...
                (cmd: SuccessfulParsedMessage<Message>, msg: Message) => {
                    ...
                    msg.channel.send(
                        createInfoEmbed(
                            `Repeat mode is ${
                                this.config.queue.repeat ? 'on' : 'off'
                            }`
                        )
                    );
                }
                ...
                if (
                    ...
                    if (reaction.message.embeds.length > 0) {
                        ...
                        if (embed) {
                            if (
                                reaction.emoji.name ===
                                    this.config.emojis.addSong &&
                                embed.url
                            ) {
                                this.logger.debug(
                                    `Emoji Click: Adding Media: ${embed.url}`
                                );
                                this.player.addMedia({
                                    type: 'youtube',
                                    url: embed.url,
                            ...
                            if (
                                reaction.emoji.name ===
                                this.config.emojis.playSong
                            ) {
                                this.logger.debug(
                                    'Emoji Click: Playing/Resuming Song'
                                );
                                this.player.play();
                            }
                            if (
                                reaction.emoji.name ===
                                this.config.emojis.pauseSong
                            ) {
