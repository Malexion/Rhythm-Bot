...
export class MediaPlayer {
    ...
    addMedia(item: MediaItem): Promise<void> {
        return new Promise((done, error) => {
            ...
            .catch((err) => {
                if (this.channel)
                    this.channel.send(
                        createErrorEmbed(`Error adding track: ${err}`)
                    );
            });
    }
    ...
    dispatchStream(stream: Readable, item: MediaItem) {
        ...
        this.dispatcher.on('error', (err) => {
            ...
            if (this.channel)
                this.channel.send(
                    createErrorEmbed(`Error Playing Song: ${err}`)
                );
        });
    ...
    play() {
        if (this.queue.length == 0 && this.channel)
            this.channel.send(
                createInfoEmbed(`Queue is empty! Add some songs!`)
            );
            ...
            if (type) {
                if (!this.playing) {
                    ...
                    if (this.channel)
                        this.channel.send(
                            createInfoEmbed(
                                `⏯️ "${this.queue.first.name}" resumed`
                            )
                        );
                }
            }
    ...
    pause() {
        if (this.playing && !this.paused && this.dispatcher) {
            ...
            if (this.channel)
                this.channel.send(
                    createInfoEmbed(`⏸️ "${this.queue.first.name}" paused`)
                );
        }
    }
        ...
        if (item) {
            if (this.playing) {
                if (this.paused) {
                    ...
                    this.status.setBanner(
                        `Now Playing: "${item.name}" Requested by: ${
                            item.requestor
                        }${
                            this.queue.length > 1
                                ? `, Up Next "${this.queue[1].name}"`
                                : ''
                        }`
                    );
                }
                ...
                this.status.setBanner(
                    `Up Next: "${item.name}" Requested by: ${item.requestor}`
                );
